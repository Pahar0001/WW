...
const ExpenseIn = z.object({
  ...
  shares: z.array(z.coerce.number().int().min(1)).optional(), // weight, or exact kopecks when exactSplit
  exactSplit: z.boolean().optional(), // true = shares are exact per-person amounts (kopecks)
});
