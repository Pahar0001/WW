 ...
 */
// tour — якорь для онбординг-тура первого визита (OnboardingTour).
const ITEMS: { href?: string; label: string; hint: string; icon: string; event?: string; tour?: string }[] = [
  { href: '/#dream-trips', label: 'Маршруты', hint: 'Готовые путешествия', icon: 'M3 7l9-4 9 4-9 4-9-4zM3 7v10l9 4 9-4V7M12 11v10', tour: 'trips' },
  { href: '/trips/new', label: 'Собрать поездку', hint: 'Свой маршрут по дням', icon: 'M12 5v14M5 12h14' },
  { href: '/order', label: 'Заказать путешествие', hint: 'Организатор соберёт за вас', icon: 'M22 2 11 13M22 2l-7 20-4-9-9-4 20-7z', tour: 'order' },
    ...
    <section className="container-vela pb-6 pt-2">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-5">
        {ITEMS.map((it) => {
          ...
          return it.event ? (
            <button key={it.label} data-tour={it.tour} onClick={() => window.dispatchEvent(new Event(it.event!))} className={cls}>
              {inner}
            </button>
          ) : (
            <Link key={it.label} data-tour={it.tour} href={it.href!} className={cls}>
              {inner}
            </Link>
          );
        })}
