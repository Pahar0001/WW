...
export function SupportWidget() {
  ...
  useEffect(() => {
    ...
    // Открытие из мобильного бургер-меню (лончер на мобильных скрыт).
    const openIt = () => setOpen(true);
    window.addEventListener('vela:open-support', openIt);
    return () => window.removeEventListener('vela:open-support', openIt);
  }, []);
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-5 right-5 z-[9000] hidden h-14 w-14 items-center justify-center rounded-full bg-aurora text-aurora-fg shadow-lg transition-transform hover:scale-105 md:flex"
      >
        {open ? '✕' : (
      ...
      {open && (
        <div className="pop-menu fixed bottom-5 right-5 z-[9000] flex h-[60vh] max-h-[520px] w-[92vw] max-w-sm flex-col overflow-hidden rounded-2xl border border-ink-line bg-ink shadow-2xl md:bottom-24" style={{ ['--pop-origin' as string]: 'bottom right' }}>
