...
/**
 * Версия кэшей приходит из адреса регистрации: `/sw.js?v=<BUILD_ID>`
 * (см. PwaProvider и next.config.js).
 *
 * Раньше здесь стояла константа 'v1', которая не менялась никогда. Из-за этого
 * обработчик `activate` — он удаляет все кэши, кроме перечисленных, — вычищал
 * ровно ничего: имена кэшей после деплоя совпадали со старыми. В PAGES
 * оставался HTML, отданный ДО деплоя, со ссылками на чанки со старыми хешами;
 * этих файлов на сервере уже нет, и первая же навигация, отданная из кэша,
 * роняла приложение с «client-side exception».
 *
 * Смена BUILD_ID меняет адрес SW → браузер ставит новый экземпляр → `activate`
 * видит незнакомые имена старых кэшей и удаляет их. Кэш сохранённых поездок
 * (TRIPS) намеренно живёт вне версии и переживает деплой.
 */
