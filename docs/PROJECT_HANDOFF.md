...
ознакомительных маршрутов по странам сообщества** (публичные; hero из Википедии). **НОВОЕ: у всех 28
маршрутов теперь есть настоящий план по дням** (RouteVariant BALANCED → Day → Place): 210 дней,
189 реальных мест — фото, координаты, описание и sourceUrl каждого места взяты из ru.wikipedia
(координаты → VERIFIED; 2 места без координат в статье → PENDING). Данные по странам/местам заданы
в `seed-countries.ts` (массив `plan` у каждой страны). Построение идемпотентно: дни создаются только
если у поездки ещё нет варианта маршрута (правки редактора не затираются); hero не перекачивается,
если уже есть. Первый сид на пустой БД тянет ~190 статей Википедии (~2–3 мин, с retry на 429).
Итого **29 публичных маршрутов** в коллекции.
...
|------|---------|--------|
| **GET** | **`/api/travel/status`** — сконфигурирован ли Travelpayouts + список городов вылета | публично |
| **GET** | **`/api/travel/plan/:slug`** `?origin=MOW&depart=YYYY-MM-DD&return=YYYY-MM-DD` — реальные цены билетов (Aviasales Data API, кэш 10 мин) + ссылки отелей с датами по городам маршрута. Приватные поездки — по опциональному Bearer | публично / опц. auth |
...
**Добавлено в сессии 2026‑07‑24 (перелёты + дни маршрутов):**
- **План по дням у всех 28 ознакомительных маршрутов** — реальные места с фото/координатами/описанием
  из Википедии (см. раздел 4). Та же структура, что использует редактор «Дни маршрута».
- **Блок «Перелёт и даты»** на странице поездки: реальные цены авиабилетов Aviasales под выбранные
  даты и город вылета, итоговая стоимость поездки (билеты VERIFIED + расходы ESTIMATED), ссылки на
  отели с датами по городам маршрута. Бэкенд: модуль `travel` (см. раздел 7).
- **Интерактивный глобус на главной:** точки-страны с реальными координатами
  (`lib/country-coords.ts`), hover — стеклянная подпись с названием страны, клик — переход на
  маршрут (`Hero3D.tsx` принимает `markers`; `GET /api/trips` теперь отдаёт `country.slug`).
  Дуги-маршруты рисуются между реальными странами. Вращение замирает при наведении.
  Перф 3D: пауза рендера вне вьюпорта (IntersectionObserver + `frameloop`), на мобильных
  1300 частиц вместо 2600. Текстовый слой героя — `pointer-events-none` (клики проходят к
  глобусу), интерактивные элементы возвращают `pointer-events-auto`.
- **Бегущая строка преимуществ** — с тонкими line-иконками (золото) и hover-подсветом.
...
```
**`TRAVELPAYOUTS_TOKEN`** — токен Travelpayouts. **Используется** модулем `travel`
(реальные цены авиабилетов Aviasales в блоке «Перелёт и даты»). Значение лежит локально в `.env`
(в `.gitignore`); прокинут в docker-compose и объявлен в `render.yaml` (`sync: false`) —
**на Render Dashboard нужно вписать значение вручную** (vela-api → Environment).
⚠️ Hotellook (отельная часть Travelpayouts) закрыт: `engine.hotellook.com` отдаёт 404,
`search.hotellook.com` редиректит на Aviasales Отели — цен по отелям через этот токен больше нет.
...
5. **`TRAVELPAYOUTS_TOKEN`** есть локально в `.env` и используется модулем `travel`, но значение
   ещё НЕ вписано в Render env → на проде блок «Перелёт и даты» будет предлагать поиск Aviasales
   без котировок, пока владелец не добавит токен в Dashboard.
...
### B. Реальные цены — Travelpayouts (СДЕЛАНО: перелёты; отели — API закрыт)
- ✅ **Авиабилеты:** модуль `backend/src/modules/travel` (Aviasales Data API `prices_for_dates`,
  кэш 10 мин, карта страна→IATA в `destinations.ts` с fallback-аэропортами). На странице маршрута —
  блок **«Перелёт и даты»** (`components/trip/TravelPlanner.tsx`): город вылета + даты → реальные
  цены туда-обратно, ссылка «Выбрать» в выдачу Aviasales, итог = билеты (VERIFIED) + расходы на
  месте (ESTIMATED из `/estimate`). Если кэш пуст (например, RU→BUD без прямых рейсов) — честный
  фолбэк со ссылкой на живой поиск Aviasales.
- ⚠️ **Отели (Hotellook) НЕВОЗМОЖНЫ:** продукт закрыт (весь `engine.hotellook.com` — 404). Вместо
  цен — живые дип-линки с выбранными датами (Яндекс Путешествия / Ostrovok / Booking) по городам
  маршрута в том же блоке; кураторские отели из админки отображаются как раньше (`HotelsSection`).
- ✅ **Монетизация (решение владельца, 24.07.2026): партнёрка + платный заказ под ключ.**
  1) `TRAVELPAYOUTS_MARKER` (env, объявлен в render.yaml/docker-compose) — партнёрский marker
  добавляется ко всем ссылкам выдачи Aviasales (комиссия с бронирований). Токен ≠ marker: marker
  берётся в кабинете Travelpayouts. 2) Заявки «Заказать путешествие» — платная услуга: админ
  назначает `priceRub` в /admin/orders, цена и ответ видны пользователю; оплата вне сайта
  (по договорённости), касса не подключалась. Подписка Premium — отложена сознательно.
- ❗ Осталось: вписать значения `TRAVELPAYOUTS_TOKEN` и `TRAVELPAYOUTS_MARKER` в Render env
  `vela-api` (ключи объявлены в render.yaml, `sync: false`).
