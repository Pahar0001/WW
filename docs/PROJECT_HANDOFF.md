...
| **GET/POST** | **`/api/assistant/status`** · **`/api/assistant/chat`** `{messages[]}` (ИИ-консультант Groq) | вошедший |
...
- **ИИ-консультант (Groq):** плавающий виджет `AssistantWidget` (снизу слева, открывается кнопкой или
  событием `vela:open-assistant` из меню главной). Бэкенд `assistant` → Groq (OpenAI-совместимый API),
  `GROQ_API_KEY`/`GROQ_MODEL` (по умолчанию `llama-3.3-70b-versatile`); без ключа — вежливая заглушка.
  Системный промпт: маршруты/визы/документы, Real Data Policy (не выдумывать цены/сроки).
- **Меню на главной** (`components/ui/HomeMenu.tsx`): быстрый доступ к разделам + карточка ИИ-консультанта.
- **Курсор:** кастомный `MagneticCursor` удалён — используется стандартный системный курсор.
- **Дизайн-пасс (премиум/софт):** смягчена палитра в `globals.css` — тёплая ivory-бумага, мягкий
  тёплый шалфейный акцент вместо сине-бирюзового (`--primary` 146°, ниже насыщенность), выше контраст
  текста; добавлены утилиты `.shadow-soft`/`.shadow-soft-lg`. Фон успокоен (`SiteScenery`/`SakuraTree`):
  меньше лепестков/облаков, слабее солнце, дерево сакуры `opacity-40`, лепестки в тёплом приглушённом
  тоне. Карточки меню на главной переоформлены (без обрезки текста, мягкая тень, ховер-подъём).
- **Загрузка файлов устойчива:** при сбое S3 (`storage.service`) — лог реальной ошибки + фолбэк на диск
  вместо 500; на клиенте (`profile`, `news`) ошибка загрузки теперь показывается тостом.
  **Важно для прода:** для Supabase `S3_REGION` должен быть реальным регионом проекта (например
  `eu-central-1`), а не `auto` — иначе аплоад падает с 500 (ломается подпись запроса).
