...
> Последнее обновление: **2026-07-27**. Ветка `main`, всё запушено и живо на проде.
> Премиальный моушн-слой, игровой мир `/vela` и переработанный глобус — **§16**:
> там что сделано, какие решения приняты и на какие грабли уже наступили.
...
(`public/globe/earth.jpg`, 435 КБ) на непрозрачной сфере + ПРОЦЕДУРНЫЙ слой облаков (см. §16.7;
`clouds.jpg` удалён — он был пустым белым файлом) + границы стран из `src/data/globe-geo.json` (Natural Earth 110m, предвычислено
...
public/globe/earth.jpg                                          # NASA Blue Marble, public domain
```
...
### 16.7. Глобус: убрана «туманность», Земля переведена на физическую модель

Владелец попросил убрать туманность на глобусе и сделать его максимально
реалистичным. Причина туманности оказалась не в настройках, а в ассете.

⚠️ **`public/globe/clouds.jpg` был ПУСТЫМ файлом: 1600×800 сплошного белого,
ни одного пикселя темнее 255** (проверено попиксельно в нативном разрешении).
Слой рисовался аддитивно с opacity 0.38, то есть просто намазывал ровную белую
пелену на весь диск. Именно это и читалось как дымка. Файл удалён.

Что сделано вместо него:
- **Облака считаются процедурно** (`makeCloudAlphaMap` в `Hero3D.tsx`) и
  работают маской прозрачности на белом ОСВЕЩЁННОМ слое, а не аддитивным
  свечением: настоящие облака ничего не излучают и гаснут на терминаторе.
  Шум сэмплируется на цилиндре (3D по cos/sin долготы) — карта сходится по шву
  0°/360°; domain warping даёт закрученные фронты; плотность зависит от широты
  по реальной климатологии (ВЗК на экваторе, разрыв в субтропиках, штормовые
  треки в умеренных широтах).
- **Убран туман сцены** (`<fog>` начинался в 7 единицах от камеры, то есть
  ровно на дальней половине шара), **солнечное гало, объёмные лучи и поле
  пыли** — в вакууме рассеивать нечего.
- **Блик солнца по океану:** маска воды строится прямо в шейдере по разнице
  каналов Blue Marble (синий заметно выше красного и зелёного), и внутри неё
  шероховатость падает до 0.11. Прежний ровный clearcoat давал лаковый отблеск
  и на пустынях — планета выглядела ёлочным шаром.
- **Атмосфера** — рэлеевский френель-ободок, гаснущий на ночной стороне
  (прежняя аддитивная оболочка светилась по всему кругу ровным ореолом).
- **Свет:** одно почти белое солнце + пепельный свет вместо сильной ambient,
  которая размывала терминатор. Экспозиция 1.22 — компромисс: у океана Blue
  Marble очень низкое альбедо, и при 1.08 золотые метки стран тонули в черноте.
- **Пост-обработка** сведена к оптике: слабый bloom с высоким порогом, зерно
  выключено полностью (плёночное зерно противоположно фотореализму).

`components/fx/Volumetric.tsx` после этого остался без единого потребителя,
кроме генератора текстуры частиц, — заменён на `components/fx/particles.ts`
(только `makeGlowTexture`, который нужен всем `pointsMaterial`: без карты он
рисует квадраты).
