...
- ❗ Осталось: вписать значение `TRAVELPAYOUTS_TOKEN` в Render env `vela-api` (ключ уже объявлен в
  render.yaml, `sync: false`). Для монетизации ссылок нужен партнёрский `marker` Travelpayouts —
  спросить у владельца (токен ≠ marker).
