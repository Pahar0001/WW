...
>
> Состояние: **2026-08-03**. Ветка `main`, HEAD = `7e119db`, рабочая копия чистая, всё запушено
> и живо на проде. Эндпоинты, модели, маршруты и переменные в этом документе сверены с кодом.
>
> **Чем занята работа прямо сейчас:** владелец дал спецификацию из 15 пунктов — превратить
> игровой мир `/vela` из красивой демонстрации в полноценное приключение и сделать его заметной
> частью сайта. Сделано 3 пункта из 15, остальные 11 расписаны в §14 с приоритетами. Это главная
> линия работ; всё остальное в §14 — фон.
...
**47 моделей и 17 enum.** Сверить одной командой:

```bash
grep -cE '^model ' backend/prisma/schema.prisma   # 47
grep -cE '^enum '  backend/prisma/schema.prisma   # 17
...
✅ **Прод живой и актуален.** HEAD `7e119db`, рабочая копия чистая, всё запушено.

**Схема БД за последние три коммита НЕ менялась** — весь игровой прогресс лежит в `localStorage`
браузера (ключ `vela_island_progress`, формат v2). `db push` запускать не нужно.

Проверено на текущем HEAD:
- `npx tsc --noEmit` — чисто; `npx next build` с нуля — чисто (33 страницы).
...
| 2 | **Физика движения**: инерция, естественный разгон и торможение, физика прыжка, реакция на наклон, скольжение на льду, замедление в воде, разное сцепление | **`SURFACE_GRIP` в `world/terrain.ts` уже есть** (лёд 0.16, песок 0.72, камень 1.05) и НЕ используется контроллером. Начинать оттуда. `surfaceAt(x,z,h,slope)` даёт материал под ногами. |
...
| 8 | **Мобильная адаптация и авто-качество**: джойстик, кнопки действий, мультитач, жесты камеры, масштабирование HUD, портрет/альбом; авто-выбор High/Medium/Low | Джойстик и кнопка прыжка есть, кнопки действия (E) на тач НЕТ — на телефоне сейчас недоступны съёмка и работы. `live.fps` считается, но на качество не влияет: авто-подстройки нет. |
